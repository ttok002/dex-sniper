...
task('uniswapV2Clone:getSwaps', 'Get swaps for the given pair after a certain block')
  .addPositionalParam('dexName', 'DEX to consider, e.g. UniswapV2')
  .addPositionalParam('pair', 'Address of the pair to analyze')
  .addParam('fromblock', 'Block number from which to start', undefined, types.int)
  .addOptionalParam(
    'nblocks',
  ...
  .setAction(
    async ({ dexName, pair, fromblock, nblocks, csv, digits0, digits1, maintoken }, hre) => {
